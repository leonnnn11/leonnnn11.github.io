...
```
这样就可以AC掉这一道题，我们用index记录儿子的数量，而index=0的时候就是叶子节点  
通过这一题基本上可以了解树形dp，以树上节点作为dp的对象，而非线性表中以数组中的元素作为dp对象  
那么我们进入下一题  
下一题：黄金矿工！  
难度：蓝题！  
下面附上题目  
...
这其实就是分组背包中的一种，我们通过是否在一条直线上，将黄金分组，同一条直线上的黄金在一组里面，而里原点更近的会在同一组的前面，而被挡住的，在同一条直线后面的黄金就排在同一组的后面。  
然后dp的时候考虑一组的时候，分别考虑取这组的第一个，第一个和第二个.....直到第一个到最后一个（全部都挖）  
如此背包dp下去就可以啦，但是问题出现了，我们要怎么把同一条直线的放到同一个组里面呢  
这里想到了排序，我们用“角度”来排序，也就是用  
```cpp
int cmd(gold a,gold b)
{
	if(a.x*b.y!=a.y*b.x)
	{
...
```
这样的排序其实就是比较x1/y1和x2/y2，也就是比较角度，如果是同一个角度的会排在一起，不同角度的会排在不一起（如果有同角度的），显然，同一个角度的x/y必定会相等。  
同一个角度的，我们依照x坐标，x坐标小的排在前面，其实也就是近的排前面。  
然后我们依次遍历各个黄金，每次检查前面有几个黄金是同组的，然后dp状态从“这个组一个都不挖”到“这个组到这个黄金为止全部都挖”，取更好的那一个作为dp值。  
这个问题就这么解决啦！！当然，我们的排序其实造成了时间浪费，看到题解说可以用平衡树去优化算法，但是前面的路，以后再来探索吧！！  
下面附上我独立完成的代码  
```cpp
#include<iostream>
using namespace std;
...
int main()
{
	...
	sort(miner.begin()+1,miner.end(),cmd);
	//for(int i=1;i<=N;i++)
	//cout<<miner[i].x<<" "<<miner[i].y<<" "<<miner[i].t<<" "<<miner[i].value<<endl;
	//return 0;
	...
	for(int i=1;i<=N;i++)
	{
		...
		for(int k=i-1;k>0;k--)
		{
			if(miner[k].x*miner[i].y==miner[k].y*miner[i].x)
			{
				m++;
			}
			else
		...
		for(int k=i;k>=i-m;k--)//此前写成了k>=k-m,k会实时变化，不能这么写
		{
			t+=miner[k].t;
			v+=miner[k].value;
		}
		for(int j=1;j<=T;j++)
		{
			...
			dp[i][j]=max(dp[i-m-1][j],dp[i-m-1][j-t]+v);
		}
	}
	cout<<dp[N][T];
	return 0;
...
```
至此，完成了总共8道集训的dp题目，集训第一天的内容正式毕业！！  
在此做一个完结撒花！！  
