...
这里复习一次upper_bound和lower_bound函数,前者是输入arr.begin(),arr.end(),m找到第一个完全大于m的数字，给出索引index，后者是找出第一个大于等于m的，我们找最长上升子序列用这个找到第一个大于等于的就可以了，如果返回的索引超出了数组，就新加进这个数字就好。  
接下来是找最长不下上升子序列，这个就没法用upper_bound和lower_bound了，我们自己手写二分查找，首先建立数组，然后每次找到第一个严格小于的新数字的，替换成新数字即可，这里的二分查找先根据要求写出>=/<或者<=/>写完以后看看是r+1/l还是r/l-1，前者mid=(l+r)/2,  
也就是剩下两个数字的时候选前面的，后者则需要mid=(r+l)/2+1,即选两个数字的后面一个，防止选前面一个，发现在范围内，然后mid=r，又来一遍，无限循环。  
下面附上代码，除了数学结论其他全部独立完成。  
```cpp
#include<iostream>
using namespace std;
#include<vector>
...
int main()
{
	...
	for(int i=0;i<len;i++)
	{
		if(raise.empty()&&unraise.empty())
		{
			raise.push_back(arr[0]);
			unraise.push_back(arr[0]);
			continue;
		}
		vector<int>::iterator it=raise.begin();
		auto change=lower_bound(it,it+raise.size(),arr[i]);
		if(change==raise.end())
		{
			raise.push_back(arr[i]);
		}
		else
		{
			*change=arr[i];	
		}
		 unsigned int r=0,l=unraise.size();
		while(r<l)
		{
			int mid=(r+l)/2;
			if(unraise[mid]>=arr[i])
			{
				r=mid+1;
			}
			else
		...
		/*for(unsigned int j=0;j<raise.size();j++)
		{
			if(arr[i]<raise[j])
			{
				raise[j]=arr[i];
			...
			if(j==raise.size()-1&&arr[i]>raise[j])
			{
				raise.push_back(arr[i]);
			}
		}*/
	...
	cout<<unraise.size()<<endl<<raise.size();
	
	
	return 0;
}
```
今天这篇博客就先总结这三题吧，收获非常大，剩下三题会在明天的博客体现
